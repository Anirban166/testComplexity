...
## Testing
The fpop algorithm is expected to possess log-linear time complexity, as empiricially derived. We can test and verify the same using testComplexity's functions:
```r
library(testComplexity)
asymptoticTimeComplexityClass(asymptoticTimings(fpop::Fpop(rnorm(data.sizes), 1), data.sizes = 10^seq(1, 4, by = 0.5), max.seconds = 0.1))
#> [1] "loglinear"
```
