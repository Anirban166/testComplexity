...
<p style="font-family: times, serif; font-size:25pt; font-style: bold" align="center">
...
As an example, lets take our function `PeakSegPDPA` and test it against different complexity classes:
```r
...
#> [1] "loglinear"
```
The above function throws no error, which means that our algorithm does follow a log-linear trend. On the other hand, if we test for the same against the linear or quadratic variants, we would get errors: (as expected)
```r
expect_linear_time({
data.vec <- rpois(N, 1)
PeakSegOptimal::PeakSegPDPA(data.vec, max.segments = 3L)}, data.sizes = 10^seq(1, 4, by = 0.5))
...
plotTimings(df, titles = list("Timings", "PeakSegOptimal::PeakSegPDPA"), line.color = "orange", point.color = "orange", line.size = 1, point.size = 1.5)  
...
plotMemoryUsage(df, titles = list("Memory usage", "PeakSegOptimal::PeakSegPDPA"), line.color = "orange", point.color = "orange", line.size = 1, point.size = 1.5)  
```
The user can add themes to the function itself (since it returns a ggplot object) to suit his/her aesthetics: <br>

```{r echo=FALSE}
knitr::include_graphics("https://raw.githubusercontent.com/Anirban166/testComplexity/master/Images/timememorycDPA.png")
```
...
As an example, consider comparing the trends of `PeakSegOptimal::PeakSegPDPA()` with the function [`PeakSegDP::cDPA()`](https://github.com/tdhock/PeakSegDP/blob/master/R/cDPA.R), which is [found](https://anirban166.github.io/testComplexity/articles/cDPA.html) to be quadratic in nature: <br>

```{r echo=FALSE}
knitr::include_graphics("https://raw.githubusercontent.com/Anirban166/RGSOC20_Tests/master/Images/finalplot.png")
```
